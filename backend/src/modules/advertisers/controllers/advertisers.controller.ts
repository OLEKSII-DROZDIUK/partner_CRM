import { Body, Controller, UseGuards, Get, Param, Post, Request, Put, HttpException, HttpStatus, ValidationPipe, UsePipes } from '@nestjs/common';
...
export class AdvertisersController {
  ...
  constructor(private readonly advertisersService: AdvertisersService,
    ...
    @Get('/all')
    private async getAllAdvertisers(@Request() req): Promise<any | null> {
    ...
    private async createAdvertiser(@Body() body: any): Promise<Advertisers> {
    ...
    @Get('/offers/:offerId')
    private async getOffersList(@Param('offerId') offerId: string): Promise<any> {
      try {
    ...
    @UseGuards(AdvertisersGuard)
    @UsePipes(new ValidationPipe())
    private async EditAdvertiser(@Body() body: any): Promise<Advertisers> {
